...
function* loadSearchedFeeds(action: ReturnType<typeof loadSearchFeedsRequest>) {
  try {
    const result = yield call(loadSearchedFeedsAPI, action.searchUrl);
    yield put({
